...
3. Create 5 tabs in HostelDB with these exact header rows (row 1 of each tab, exact spelling/order). **Correction: the backend does NOT actually read columns by name** — `getSheetRows()`/`writeRowAt()` in `api/_lib/sheets.js` read/write a fixed `A2:<lastCol>` range purely by COLUMN POSITION, matching each code-side `*_COLUMNS` array index to a letter. The header row is for human reference only; code never looks at it. This means column ORDER must match the arrays in `api/_lib/schema.js` exactly, and any future column must be appended at the end, never inserted/reordered/removed — doing either would silently misalign every existing row.
   ...
   **Applications, again** — updated 2026-08-24 to add distance-based allocation tiebreaking: `DistanceFromResidenceKm` (a self-declared number, same trust model as `CategoryResidence`) and `AddressProofDriveLink` (same pattern as the other five `*DriveLink` document columns). Appended at the very end, after `GuardianLandmark` — if your live sheet predates this, add these 2 header cells there; don't touch anything before it. Existing rows with no value in `DistanceFromResidenceKm` aren't blocked — `_lib/allocation.js`'s `compareCandidates()` treats a blank/non-numeric/zero distance as "sorts last within its tier" rather than erroring, so old test data keeps working, it just won't benefit from the distance tiebreak until filled in.
